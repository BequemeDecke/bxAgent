...
def extract_written_files(
    ...
    relative_file_paths: map[str] = (
        msg.content[updated_file_index:] for msg in write_file_messages
    )
    absolute_file_paths: map[Path] = (
        workspace_path / path for path in relative_file_paths
    )
    return list(absolute_file_paths)


class TrackWrittenFilesMiddleware(AgentMiddleware[TransformationClassAgentState]):
    """
    Middleware for the Transformation Class agent that manages the state of the agent. This middleware can be used to track the files that have been written by the Transformation Class agent during its operation.
    """
    ...
    def after_agent(self, state: TransformationClassAgentState, runtime):
        """
        ...
        logger.info(f"Agent has written the following files: {written_files}")
        return TransformationClassAgentState(written_files=written_files)
